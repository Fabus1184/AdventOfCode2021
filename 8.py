...
valid = ["cagedb", "ab", "gcdfa", "fbcad", "eafb", "cdfbe", "cdfgeb", "dab", "acedgfb", "cefabd"]

valid = ["".join(sorted(i)) for i in valid]

al = "abcdefg"

combs = list(itertools.permutations(al, len(al)))

def check(ss):
    available = list(valid)
    for k in [("".join(sorted(k))) for k in ss.split(" ")]:
...
for k in kek:
    print("%i / %i : %i" % (kek.index(k), len(kek), out))
    for c in combs:
        ss = k.split(" | ")[0]
        ...
        rep = ss.maketrans(al, rep)    
        ss = ss.translate(rep)

        ss = (" ".join(["".join(sorted(i)) for i in ss.split(" ")]))

        if(check(ss)):
            numbar = ""
            for i in k.split(" | ")[1].split(" "):
                soos = i.translate(rep)
                soos = (" ".join(["".join(sorted(ef)) for ef in soos.split(" ")]))        
                numbar += str(valid.index(soos))
            out += int(numbar)
            break
print(out)
